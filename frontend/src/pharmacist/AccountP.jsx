...
const AccountP = () => {
  ...
  const handleUpdate = async(e)=>{
      ...
      if(input.username===""||input.email===""||input.password===""||input.passConfirm===""){
          ...
          try {
              const res = await axios.put(`http://localhost:4000/api/pms/v1/pharmacists/`,{
                  ...input
          ...
          <form>
              <div className="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                  <div>
                      ...
                      <input name="username" onChange={handleChange} id="username" defaultValue={user.pharmacist_name} type="text" className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring"/>
                  </div>
      
                  <div>
                      <label className="text-gray-700 dark:text-gray-200" htmlFor="emailAddress">Email Address</label>
                      <input name="email" onChange={handleChange} id="emailAddress" defaultValue={user.pharmacist_email} type="email" className="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring"/>
                  </div>
